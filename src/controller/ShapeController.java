...
public abstract class ShapeController implements ModelListener {
    ...
    public ShapeController(ShapeModel model, Canvas canvas) {
        this.model = model;
        this.canvas = canvas;
    ...
    public static Rectangle getBigBoundsForModel(ShapeModel model) {
    ...
    public boolean containsPoint(Point pt) {
        ...
        if(bounds.width == 0 && Math.abs(pt.x - bounds.x) <= 3 && pt.y <= bounds.y + bounds.height && pt.y >= bounds.y) {
            return true;
        }
        if(bounds.height == 0 && Math.abs(pt.y - bounds.y) <= 3 && pt.x >= bounds.x && pt.x <= bounds.x + bounds.width) {
            return true;
        }
        return false;
    } //End containsPoint
    ...
    protected void drawBorders(Graphics2D g2, ShapeController shape) {
        if(shape instanceof RectController) {
    ...
    abstract public ShapeModel getModel();
